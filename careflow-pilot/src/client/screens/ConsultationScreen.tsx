...
import { clinicalNoteDraftInputSchema, medicationDecisionDraftInputSchema, type ClinicalNoteDraftInput, type MedicationDecisionDraftInput, type PatientSnapshotDto, type ReviewAllergyPayload, type SignClinicalNoteAmendmentBody, type SnapshotSource, type VisitWorkspaceDto } from "../../shared/contracts";
...
function workspaceDraftStamp(data: VisitWorkspaceDto) { return `${data.consultationDraft.note?.revision ?? 0}:${data.consultationDraft.medicationDecision?.revision ?? 0}`; }
...
export function ConsultationScreen(): ReactElement {
  ...
  const [value, setValue] = useState<ConsultationFormValue>(blankValue); const [stamp, setStamp] = useState(""); const [dirty, setDirty] = useState(false); const [conflict, setConflict] = useState(false); const [allergyBlocked, setAllergyBlocked] = useState(false); const [showAllergy, setShowAllergy] = useState(false); const [showConfirm, setShowConfirm] = useState(false); const [evidenceAction, setEvidenceAction] = useState<"amend" | "revise" | null>(null); const [amendment, setAmendment] = useState({ content: "", reason: "" });
  ...
  const data = workspace.data; const editable = data.allowedActions.includes("SAVE_DRAFT") && auth.session?.user.role === "doctor"; const draftValue = saveableValue(value); const validDraft = clinicalNoteDraftInputSchema.safeParse(draftValue.note).success && medicationDecisionDraftInputSchema.safeParse(draftValue.medicationDecision).success; const validDecision = decisionValid(value.medicationDecision); const status = data.visit.status === "CONSULTING" ? { label: "กำลังตรวจ", tone: "active" as const } : { label: data.visit.status, tone: "waiting" as const };
  const reloadWorkspace = async () => { const result = await workspace.refetch(); if (!result.isSuccess || !result.data) return; setStamp(workspaceDraftStamp(result.data)); setConflict(false); setAllergyBlocked(false); allergy.reset(); saveAttempt.current = null; finalizeAttempt.current = null; if (!dirty) setValue(formValue(result.data)); };
  const saveDraft = () => { if (!validDraft || conflict) return; saveAttempt.current ??= createSaveDraftAttempt(data, draftValue); save.mutate({ visitId: data.visit.id, attempt: saveAttempt.current }, { onError: (error) => { if (isApiError(error) && error.status === 409) { setConflict(true); saveAttempt.current = null; } }, onSuccess: () => { saveAttempt.current = null; setDirty(false); setConflict(false); } }); };
  ...
  return <div className="flow-page consultation-page">
    ...
    <div className="clinical-workspace-grid"><aside className="consultation-patient-rail" aria-label="บริบทผู้ป่วย"><div className="care-card patient-identity"><div className="patient-header"><div className="patient-header-copy"><strong>{data.patient.displayName}</strong><span>HN {data.patient.hn}</span></div><StatusBadge tone={status.tone}>{status.label}</StatusBadge></div><div className="allergy-summary"><span>ประวัติแพ้ยา</span><strong>{data.patientSnapshot.allergy.state}</strong><ActionButton type="button" variant="secondary" onClick={() => { allergy.reset(); setAllergyBlocked(false); setShowAllergy(true); }} disabled={!data.allowedActions.includes("REVIEW_ALLERGY")}>ทบทวนประวัติแพ้</ActionButton></div><Link className="care-button care-button-secondary" to="/queue">กลับคิวผู้ป่วย</Link></div><PatientSnapshot snapshot={data.patientSnapshot} /></aside>
      <div className="consultation-clinical-content"><section className="care-card consultation-current-visit" aria-label="ข้อมูล Visit ปัจจุบัน"><SectionHeading icon={Stethoscope} title="ข้อมูล Visit ปัจจุบัน" description={`อาการสำคัญ: ${data.intake.chiefComplaint}`} /><div className="consultation-meta"><span>Visit {data.visit.id}</span><span>revision {data.visit.revision}</span><StatusBadge tone={status.tone}>{status.label}</StatusBadge></div><div className="vitals-summary"><span>อุณหภูมิ <strong>{vital(data.intake.vitals.temperatureC, " °C")}</strong></span><span>ความดัน <strong>{data.intake.vitals.systolicMmhg === null || data.intake.vitals.diastolicMmhg === null ? "—" : `${data.intake.vitals.systolicMmhg}/${data.intake.vitals.diastolicMmhg}`}</strong></span><span>ชีพจร <strong>{vital(data.intake.vitals.heartRateBpm, " ครั้ง/นาที")}</strong></span><span>SpO₂ <strong>{vital(data.intake.vitals.spo2Percent, "%")}</strong></span><span>น้ำหนัก <strong>{vital(data.intake.vitals.weightKg, " กก.")}</strong></span><span>ส่วนสูง <strong>{vital(data.intake.vitals.heightCm, " ซม.")}</strong></span></div><section className="clinical-evidence" aria-label="หลักฐานจาก Intake"><SectionHeading icon={ClipboardCheck} title="หลักฐานจาก Intake" description="ข้อมูลนี้มาจาก snapshot ที่บันทึกแล้ว" /><div className="evidence-grid"><div><span>ผู้บันทึก</span><strong>{data.intake.recordedBy.displayName}</strong></div><div><span>เวลาบันทึก</span><strong>{formatThaiDateTime(data.intake.recordedAt)}</strong></div><div><span>มาถึงคลินิก</span><strong>{formatThaiDateTime(data.visit.arrivedAt)}</strong></div><div><span>เริ่มห้องตรวจ</span><strong>{data.visit.startedAt ? formatThaiDateTime(data.visit.startedAt) : "ยังไม่เริ่ม"}</strong></div></div></section></section>
        {data.signedClinicalNote && data.medicationDecision ? <><SignedClinicalEvidence note={data.signedClinicalNote} decision={data.medicationDecision} amendments={data.amendments} onAmend={data.allowedActions.includes("AMEND_NOTE") ? () => setEvidenceAction("amend") : undefined} onRevise={data.allowedActions.includes("REVISE_MEDICATION_DECISION") ? () => setEvidenceAction("revise") : undefined} /><div className="signed-next-actions">{data.visit.status === "AWAITING_ORDER_REVISION" ? <strong>รอแก้ไขการตัดสินใจยา</strong> : data.visit.status === "AWAITING_PREPARATION" ? <><span>การจัดยายังไม่เปิดใน Pilot</span><Link className="care-button care-button-secondary" to={`/dispensing/${data.visit.id}`}>ไปหน้าจัดยา (ยังไม่พร้อม)</Link></> : data.visit.status === "AWAITING_CHARGE" ? <><span>การชำระเงินยังไม่เปิดใน Pilot</span><Link className="care-button care-button-secondary" to={`/checkout/${data.visit.id}`}>ไปหน้าชำระเงิน (ยังไม่พร้อม)</Link></> : <span>ขั้นตอนถัดไปยังไม่พร้อมใน Pilot</span>}</div></> : <section className="care-card consultation-note-panel" aria-label="Clinical Note"><SectionHeading icon={FileSignature} title="Clinical Note" description="บันทึกโดยแพทย์เท่านั้น" />{conflict ? <div role="alert" className="conflict-alert">ข้อมูลเวอร์ชันปัจจุบันเปลี่ยนแปลงแล้ว กรุณาโหลดข้อมูลล่าสุดก่อนลงนาม <ActionButton type="button" variant="secondary" onClick={() => void reloadWorkspace()}>โหลดข้อมูลล่าสุด</ActionButton></div> : null}{save.error ? <p className="field-error" role="alert">{message(save.error)}</p> : null}{finalize.error ? <p className="field-error" role="alert">{message(finalize.error)}</p> : null}<ClinicalNoteEditor value={value.note} errors={editorErrors} onChange={(note) => changeValue({ ...value, note })} disabled={!editable || save.isPending || finalize.isPending} /><SectionHeading title="การตัดสินใจเรื่องยา" description="ต้องเลือกคำสั่งยา หรือระบุว่าไม่สั่งยา" /><MedicationDecisionEditor value={value.medicationDecision} errors={editorErrors} onChange={(medicationDecision) => changeValue({ ...value, medicationDecision })} disabled={!editable || save.isPending || finalize.isPending} />{!validDecision ? <p className="field-hint">บันทึกร่างได้ และต้องระบุรายการยาพร้อมวิธีใช้ หรือเหตุผลที่ไม่สั่งยาก่อนลงนาม</p> : null}<div className="consultation-actions"><ActionButton type="button" variant="secondary" onClick={saveDraft} disabled={!editable || conflict || save.isPending || !validDraft}>{save.isPending ? "กำลังบันทึก…" : "บันทึกร่าง"}</ActionButton><ActionButton type="button" onClick={() => setShowConfirm(true)} disabled={!editable || conflict || !validDecision || !data.consultationDraft.note || !data.consultationDraft.medicationDecision}>ลงนามและส่งต่อ</ActionButton></div></section>}</div></div>
    {showAllergy ? <AllergyReviewDialog allergy={data.patientSnapshot.allergy} onClose={() => { allergy.reset(); setAllergyBlocked(false); setShowAllergy(false); }} onSave={reviewAllergy} pending={allergy.isPending} error={allergy.error ? message(allergy.error) : undefined} blocked={allergyBlocked} onReload={() => void reloadWorkspace()} reloadPending={workspace.isFetching} /> : null}
