...
export function ConsultationScreen(): ReactElement {
  ...
  const [value, setValue] = useState<ConsultationFormValue>(blankValue); const [stamp, setStamp] = useState(""); const [dirty, setDirty] = useState(false); const [conflict, setConflict] = useState(false); const [showAllergy, setShowAllergy] = useState(false); const [showConfirm, setShowConfirm] = useState(false); const [evidenceAction, setEvidenceAction] = useState<"amend" | "revise" | null>(null); const [amendment, setAmendment] = useState({ content: "", reason: "" });
  ...
  const reloadWorkspace = async () => { const result = await workspace.refetch(); if (!result.isSuccess || !result.data) return; setStamp(workspaceDraftStamp(result.data)); setConflict(false); saveAttempt.current = null; finalizeAttempt.current = null; if (!dirty) setValue(formValue(result.data)); };
