...
function ConsultationWorkspace({ visitId }: { visitId: string }): ReactElement {
  ...
  const submitAmendment = () => { if (staleWorkspace || !consultationAuthorized || !data.signedClinicalNote || !amendment.content.trim() || !amendment.reason.trim()) return; amendmentAttempt.current ??= createCommandAttempt({ amendment: data.amendments.length }, amendment); amend.mutate({ visitId: data.visit.id, noteId: data.signedClinicalNote.id, attempt: amendmentAttempt.current }, { onSuccess: () => { amendmentAttempt.current = null; setEvidenceAction(null); } }); };
  ...
  return <div className="flow-page consultation-page">
    ...
    <div className="clinical-workspace-grid"><aside className="consultation-patient-rail" aria-label="บริบทผู้ป่วย"><div className="care-card patient-identity"><div className="patient-header"><div className="patient-header-copy"><strong>{data.patient.displayName}</strong><span>HN {data.patient.hn}</span></div><StatusBadge tone={status.tone}>{status.label}</StatusBadge></div><div className="allergy-summary"><span>ประวัติแพ้ยา</span><strong>{allergyStateLabelTh(data.patientSnapshot.allergy.state)}</strong><ActionButton type="button" variant="secondary" onClick={() => { allergy.reset(); setAllergyBlocked(false); setShowAllergy(true); }} disabled={staleWorkspace || !journeyAllows("REVIEW_ALLERGY")}>ทบทวนประวัติแพ้</ActionButton></div><Link className="care-button care-button-secondary" to="/queue">กลับคิวผู้ป่วย</Link></div><PatientSnapshot snapshot={data.patientSnapshot} /></aside>
      <div className="consultation-clinical-content"><section className="care-card consultation-current-visit" aria-label="ข้อมูล Visit ปัจจุบัน"><SectionHeading icon={Stethoscope} title="ข้อมูล Visit ปัจจุบัน" description={`อาการสำคัญ: ${data.intake.chiefComplaint}`} /><div className="consultation-meta"><span>Visit {data.visit.id}</span><span>revision {data.visit.revision}</span><StatusBadge tone={status.tone}>{status.label}</StatusBadge></div><div className="vitals-summary"><span>อุณหภูมิ <strong>{vital(data.intake.vitals.temperatureC, " °C")}</strong></span><span>ความดัน <strong>{data.intake.vitals.systolicMmhg === null || data.intake.vitals.diastolicMmhg === null ? "—" : `${data.intake.vitals.systolicMmhg}/${data.intake.vitals.diastolicMmhg}`}</strong></span><span>ชีพจร <strong>{vital(data.intake.vitals.heartRateBpm, " ครั้ง/นาที")}</strong></span><span>SpO₂ <strong>{vital(data.intake.vitals.spo2Percent, "%")}</strong></span><span>น้ำหนัก <strong>{vital(data.intake.vitals.weightKg, " กก.")}</strong></span><span>ส่วนสูง <strong>{vital(data.intake.vitals.heightCm, " ซม.")}</strong></span></div><section className="clinical-evidence" aria-label="หลักฐานจาก Intake"><SectionHeading icon={ClipboardCheck} title="หลักฐานจาก Intake" description="ข้อมูลนี้มาจาก snapshot ที่บันทึกแล้ว" /><div className="evidence-grid"><div><span>ผู้บันทึก</span><strong>{data.intake.recordedBy.displayName}</strong></div><div><span>เวลาบันทึก</span><strong>{formatThaiDateTime(data.intake.recordedAt)}</strong></div><div><span>มาถึงคลินิก</span><strong>{formatThaiDateTime(data.visit.arrivedAt)}</strong></div><div><span>เริ่มห้องตรวจ</span><strong>{data.visit.startedAt ? formatThaiDateTime(data.visit.startedAt) : "ยังไม่เริ่ม"}</strong></div></div></section></section>
        {data.signedClinicalNote && data.medicationDecision ? <>
          <SignedClinicalEvidence note={data.signedClinicalNote} decision={data.medicationDecision} amendments={data.amendments} onAmend={!staleWorkspace && consultationAuthorized ? () => setEvidenceAction("amend") : undefined} onRevise={!staleWorkspace && consultationAuthorized ? () => setEvidenceAction("revise") : undefined} />
          ...
          <div className="consultation-actions">{consultationAuthorized ? <><ActionButton type="button" variant="secondary" onClick={saveDraft} disabled={!editable || conflict || staleWorkspace || save.isPending || !validDraft}>{save.isPending ? "กำลังบันทึก…" : "บันทึกร่าง"}</ActionButton><ActionButton type="button" onClick={() => setShowConfirm(true)} disabled={!canSign}>ลงนามและส่งต่อ</ActionButton></> : <p className="field-hint" role="status">รอสิทธิ์ห้องตรวจจากเส้นทางผู้ป่วยล่าสุด</p>}</div>
        </section>}</div></div>
    ...
    {evidenceAction === "amend" ? <div className="dialog-backdrop"><section className="care-card sign-dialog" role="dialog" aria-modal="true" aria-label="เพิ่มคำแก้ไข"><h2>เพิ่มคำแก้ไข</h2><TextAreaField label="รายละเอียด" value={amendment.content} onChange={(event) => { amendmentAttempt.current = null; setAmendment({ ...amendment, content: event.target.value }); }} disabled={amend.isPending || staleWorkspace || !consultationAuthorized} /><TextAreaField label="เหตุผล" value={amendment.reason} onChange={(event) => { amendmentAttempt.current = null; setAmendment({ ...amendment, reason: event.target.value }); }} disabled={amend.isPending || staleWorkspace || !consultationAuthorized} />{amend.error ? <p className="field-error">{message(amend.error)}</p> : null}<div className="dialog-actions"><ActionButton type="button" variant="secondary" onClick={() => setEvidenceAction(null)} disabled={amend.isPending}>ยกเลิก</ActionButton><ActionButton type="button" onClick={submitAmendment} disabled={staleWorkspace || !consultationAuthorized || amend.isPending || !amendment.content.trim() || !amendment.reason.trim()}>ลงนาม</ActionButton></div></section></div> : null}
    {evidenceAction === "revise" && data.medicationDecision ? <MedicationDecisionRevisionDialog decision={data.medicationDecision} expectedRevisions={{ visit: data.visit.revision, patient: data.patient.revision, medicationDecision: data.medicationDecision.version }} pending={revise.isPending || staleWorkspace || !consultationAuthorized} error={revise.error ? message(revise.error) : staleWorkspace || !consultationAuthorized ? "ข้อมูลห้องตรวจหรือสิทธิ์ Journey อาจไม่เป็นปัจจุบัน" : undefined} onCancel={() => setEvidenceAction(null)} onSubmit={(attempt) => { if (!staleWorkspace && consultationAuthorized) revise.mutate({ visitId: data.visit.id, attempt }, { onSuccess: () => setEvidenceAction(null) }); }} /> : null}
  </div>;
}
