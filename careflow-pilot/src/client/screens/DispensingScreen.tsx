...
import { createCompletePreparationAttempt, createConfirmAllocationAttempt, createHandoffAttempt, createReleaseAttempt, createReserveDispensingAttempt, useCompletePreparation, useConfirmAllocation, useDispensingPickList, useHandoff, useRelease, useReserveDispensing, type ReserveDispensingAttempt } from "../features/dispensing";
...
export function DispensingScreen(): ReactElement {
  ...
  const reserve = useReserveDispensing(); const confirm = useConfirmAllocation(); const complete = useCompletePreparation(); const release = useRelease(); const handoff = useHandoff();
  const scannerRef = useRef<HTMLInputElement>(null);
  const reserveAttempt = useRef<ReserveDispensingAttempt | null>(null);
  const [scan, setScan] = useState(""); const [manualReason, setManualReason] = useState(""); const [localError, setLocalError] = useState("");
  ...
  const mutationError = reserve.error || confirm.error || complete.error || release.error || handoff.error; const message = localError || (mutationError ? errorMessage(mutationError) : "");
