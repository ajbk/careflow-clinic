...
export function DispensingScreen(): ReactElement {
  ...
  const reserveAttempt = useRef<ReserveDispensingAttempt | null>(null); const abandonAttempt = useRef<AbandonPreparationAttempt | null>(null); const rejectAttempt = useRef<RejectAttempt | null>(null);
  ...
  const canAbandonPreparation = journeyAllows("ABANDON_PREPARATION");
  const canReleaseMedication = journeyAllows("RELEASE_MEDICATION");
  const canRejectPreparation = journeyAllows("REJECT_PREPARATION");
  ...
  const abandonPreparation = () => { if (!preparation || !canAbandonPreparation || !abandonReason.trim()) return; setLocalError(""); abandonAttempt.current ??= createAbandonPreparationAttempt(data, abandonReason); abandon.mutate({ visitId: data.visit.id, attempt: abandonAttempt.current }, { onSuccess: () => { abandonAttempt.current = null; setAbandonReason(""); } }); };
  ...
  const rejectPreparation = () => { if (!canRejectPreparation || !rejectReason.trim()) return; setLocalError(""); rejectAttempt.current ??= createRejectAttempt(data, rejectReason); reject.mutate({ visitId: data.visit.id, attempt: rejectAttempt.current }, { onSuccess: () => { rejectAttempt.current = null; setRejectReason(""); } }); };
