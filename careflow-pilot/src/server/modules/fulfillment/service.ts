...
export function createFulfillmentService(input: FulfillmentServiceOptions): FulfillmentService {
  ...
  return {
    ...
    completePreparation(tx, actor, visitId, visitRevision, preparationRevision, preparationId, reservationId) {
      ...
      const print = tx.select().from(fulfillmentLabelPrintEvents).where(eq(fulfillmentLabelPrintEvents.labelVersionId, prep.labelVersionId)).orderBy(desc(fulfillmentLabelPrintEvents.sequence)).get();
      if (!print || print.sequence < prep.minimumPrintSequence) throw new ApiError({ code: "LABEL_PRINT_REQUIRED", messageTh: "ต้องมีคำขอพิมพ์ฉลากรอบใหม่ก่อนเสร็จสิ้นการเตรียมยา" });
      const now = clock().toISOString();
